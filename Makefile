...
PACKAGE := django_identifiers
DIST_NAME := django-identifiers
...
check: format-check lint typecheck django-check test ## Run all local validation checks
...
install-wheel: ## Install the built wheel into a temporary clean environment
	...
	test_dir="/tmp/$(PACKAGE)-wheel-test"; \
	rm -rf "$$test_dir"; \
	...
	"$$test_dir/bin/python" -c \
		'import django; import django_identifiers; from importlib.metadata import version; print("Installed:", version("$(DIST_NAME)")); print("Django:", django.get_version()); print("Package:", django_identifiers.__file__)'
...
release-check: ## Run full release validation
	...
	echo "✅ Wheel contains typing marker"
	$(MAKE) install-wheel
